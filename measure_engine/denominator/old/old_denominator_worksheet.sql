 ...
 */
SELECT *
FROM mimiciii.admissions AS inpatientencounter
WHERE UPPER(admission_type) != 'NEWBORN'
  AND DATE_PART('day', dischtime - admittime) <= 120 -- relevantPeriod in days
/*
 Definitions:
...
SELECT *
FROM mimiciii.admissions AS inpatientencounter
WHERE NOT EXISTS (SELECT hadm_id
                  FROM mimiciii.diagnoses_icd AS encounterdiagnoses
                  WHERE encounterdiagnoses.hadm_id = inpatientencounter.hadm_id
