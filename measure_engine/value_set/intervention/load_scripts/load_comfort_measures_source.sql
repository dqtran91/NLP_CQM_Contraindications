...
FROM (SELECT DISTINCT 'http://hl7.org/fhir/sid/icd-9-cm' AS source_system,
      ...
      WHERE LOWER(long_title) = 'encounter for palliative care') AS a
WHERE NOT EXISTS (SELECT 1 FROM value_set.comfort_measures AS b WHERE b.source_code = a.source_code);
